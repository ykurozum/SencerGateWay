...
class AuthenticationDelegate(DefaultDelegate):
    ...
    def handleNotification(self, hnd, data):
        ...
        #0x0041
            ...
            print "1>" + str(data.encode("hex")) + " len:" + str(len(data))
        #Value
        elif hnd == 0x3e:
            print "2>" + str(data.encode("hex"))             
            #self.disconnect()             
                ...
                #print "Need retry"
            #else:
                #self.device._log.error("3>Unhandled Response " + hex(hnd) + ": " +
                                  # str(data.encode("hex")) + " len:" + str(len(data)))
        else:
            self.device._log.error("4>Unhandled Response " + hex(hnd) + ": " +
                                   str(data.encode("hex")) + " len:" + str(len(data)))
            #self.disconnect() 
